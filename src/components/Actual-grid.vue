<template>
  <div>
    ...
    <div class="command-input">
      <label for="commands">Introduce comandos (ej. FRRLRFF):</label>
      <input type="text" id="commands" v-model="commands" />
      <button @click="executeCommands">Ejecutar Comandos</button>
    </div>
    
    <div class="move-to-destination">
      <button @click="moveToDestination">Mover al destino</button>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      commands: '', // Campo para comandos
    };
  },
  methods: {
    moveToDestination() {
      // Implementar la lógica para mover el rover según coordenadas dadas
      // (esto puede ser para sencillamente moverlo al mismo lugar que se estableció)
      const destX = this.startX;
      const destY = this.startY;

      const interval = setInterval(() => {
        if (this.x < destX) this.x++;
        else if (this.x > destX) this.x--;

        if (this.y < destY) this.y++;
        else if (this.y > destY) this.y--;

        if (this.x === destX && this.y === destY) {
          clearInterval(interval);
        }
      }, 200); // Control de la velocidad del movimiento
    },
    setInitialPosition() {
      if (this.startX >= 0 && this.startX <= 29 && this.startY >= 0 && this.startY <= 29) {
        this.x = this.startX; // Establecer la posición inicial del rover
        this.y = this.startY;
      } else {
    ...
    executeCommands() {
    // Recorre cada comando en el campo de entrada
...
.command-input {
  margin-top: 20px; /* Espacio entre el tablero y el campo de comandos */
  font-family: Arial, sans-serif; /* Fuente del texto */
}
